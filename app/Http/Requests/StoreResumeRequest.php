...
class StoreResumeRequest extends FormRequest
{
    ...
    {
        return [
            'slug' => ['required', 'string', 'max:255', 'alpha_dash', 'unique:resumes,slug'],
            'language' => ['required', 'string', 'size:3', Rule::in(['eng', 'por'])],
            'version' => [
                'required',
                'string',
                'max:20',
                'regex:/^v\d+$/',
                Rule::unique('resumes')->where(fn ($query) => $query
            ...
            'courses.*.issuer' => ['nullable', 'string', 'max:255'],
            'courses.*.date' => ['nullable', 'string', 'max:20'],
            'courses.*.url' => ['nullable', 'url', 'max:255'],
